...
if __name__ == '__main__':
    # df = pd.read_csv('../data/simulation_data_5.csv', index_col=0)
    # df = pd.read_csv('../data/raw_data.csv',index_col=0)
    # df = THI_to_GHIDNI(df)
    # # df.to_csv('../data/simulation_data.csv')
    # df.to_csv('../data/raw_data.csv')

    data_name = '20181107_20190417'
    df = pd.read_csv('../data/JMA_data_{}.csv'.format(data_name), index_col=0)
    df.index = pd.to_datetime(df.index)
    df = THI_to_GHIDNI(df)
    df = solar_heat_gain_cal(df)
    df.to_csv('../data/JMA_data_{}.csv'.format(data_name))
