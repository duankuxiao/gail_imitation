...
class PID_control():
    def __init__(self, max=5000, min=0, Kp=360, Ki=300, Kd=50):
        self.max = max
        self.min = min
        self.Kp = Kp  # 比例增益
        self.Ki = Ki  # 微分增益
...
class BaseEnv(gym.Env):
    ...
    def _set_up(self, rl_config):
        ...
        self.current_fea = ['Ti', 'temp_target', 'L', 'Tset', 'switch', 'battery', 'charge', 'Ec_ac', 'Ec_other', 'Ec_pv', 'Ec_sell', 'Ec_buy', 'Ec_demand', 'Ec_true', 'price',
                            'cost', 'CO2', ]
    ...
    def _read_inputs(self, flag) -> pd.DataFrame:
        ...
        inputs['Ec_other'] = inputs['Ec_demand'] * 0.25
        inputs['phi_i'] = inputs['phi_i'] * 0.3
        return inputs
    ...
    def _update_state_RC(self, t: int, action: np.ndarray) -> (dict, dict, dict):
        ...
        current_state['switch'], current_state['Tset'], current_state['L'], current_state['Ec_ac'] = self._ac_action(action[0], action[1], current_state['Ti'], current_state['To'])
        ...
        current_state['cost'] = (self.dt / 3600) * current_state['price'] * current_state['Ec_true'] / 1000 if current_state['Ec_true'] >= 0 else (self.dt / 3600) * 15 * \
                                                                                                                                                  current_state['Ec_true'] / 1000
        ...
        fea_variables = {'cost': current_state['cost'], 'Ti': next_state['Ti'], 'home': next_state['home'], 'temp_target': next_state['temp_target'],
                         'battery': current_state['battery'],
                         'Ec_pv': current_state['Ec_pv'], 'Ec_demand': current_state['Ec_demand'], 'Ec_sell': current_state['Ec_sell'], 'Ec_buy': current_state['Ec_buy'],
                         'Ec_true': current_state['Ec_true'], 'charge': current_state['charge'], 'CO2': current_state['CO2']}
    ...
    def _battery_action(self, charge: np.float32 = 0.0, pv: float = 0.0, ec_total: float or int = 0.0, flag: str = 'rl') -> (float, float, float):  # 0 <= self.battery_power <= 20
        if flag == 'rl':
            # TODO pv
            charge = np.clip(charge, -min(float(self.battery_power_init) * 12, self.discharge_capacity),
            ...
            if pv > 0:
                charge = np.clip(pv - ec_total, 0, min((self.battery_capacity - self.battery_power_init) * 12, pv, self.charge_capacity))
                charge *= 0.97
            else:
                charge = -min(float(self.battery_power_init) * 12, ec_total, self.discharge_capacity)
                charge *= 0.97
            battery_power = np.clip(self.battery_power_init + charge * (self.dt / 3600), 0, self.battery_capacity)
            self.battery_power_init = battery_power
    ...
    def _ac_action(self, switch: int = -1, load: float = 0.0, Ti: float = 22, To: float = 16.0) -> (int, int, float, float):
    ...
    def evaluation(self, res, output=False):
        ...
        if self.price == 'fix':
            sr_baseline, cost_baseline = 0.9197, 33977  # 0.7773 25303
        elif self.price == 'normal':
            sr_baseline, cost_baseline = 0.9197, 11194  # 0.7773 8772
        else:
            sr_baseline, cost_baseline = 0.9197, 24377  # 0.7773 -2890
        selected_rows = res[res['home'] == 1]
        rate = round((1-res['Ec_sell'].sum() / res['Ec_pv'].sum()) * 100,2)
        mape = mean_absolute_percentage_error(selected_rows['temp_target'], selected_rows['Ti'])
        mse = mean_squared_error(selected_rows['temp_target'], selected_rows['Ti'])
        ...
        count_within_range = ((filtered_df['Ti'] - filtered_df['temp_target']).abs() <= self.T_range).sum()
        ...
        metrics = {'mape': mape, 'mse': mape, 'sr': sr, 'Ec_demand': Ec_demand,'Ec_true':Ec_true, 'cost': cost, 'sr_improve': sr_eval, 'cost_improve': cost_eval,'rate':rate,'CO2': CO2 }
        if output:
    ...
    def res_figure(self, res):
        ...
        plt.plot(res['temp_target'], color="chartreuse", label='Target')
